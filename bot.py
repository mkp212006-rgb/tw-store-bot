...
def texto_pagamento(pedido: dict) -> str:
    ...
    if pedido.get("mp_qr_code"):
        ...
        return (
            "💳 *Pagamento do Pedido*\n\n"
            "Seu pedido já está separado. Agora é só realizar o pagamento pelo Pix abaixo.\n\n"
            "✅ *Como finalizar*\n"
            "1️⃣ Toque no botão *Copiar Pix*\n"
            "2️⃣ Abra o app do seu banco\n"
            "3️⃣ Pague o valor exato do pedido\n"
            "4️⃣ Aguarde a confirmação automática do Mercado Pago\n\n"
            "⚠️ Não precisa enviar comprovante neste pedido. Assim que o pagamento for aprovado, o bot continua automaticamente.\n\n"
            ...
            "Depois de pagar, aguarde alguns segundos. Se precisar, toque em *Já paguei / verificar*."
        )

    return (
        "💳 *Pagamento do Pedido*\n\n"
        "Seu pedido foi recebido. Confira as informações abaixo e faça o Pix com o valor exato.\n\n"
        "🔑 *Chave Pix*\n"
        f"`{md(PIX_CHAVE)}`\n\n"
        f"💰 *Valor exato:* R$ {md(pedido['valor'])}\n"
        f"🆔 *ID do pedido:* `{md(pedido.get('pedido_id', ''))}`\n\n"
        ...
        f"• Link/@ enviado: {md(pedido['link'])}\n\n"
        "✅ *Após pagar*\n"
        "Envie o comprovante em imagem aqui na conversa.\n"
