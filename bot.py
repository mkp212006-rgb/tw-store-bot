...
def normalizar_quantidade_cliente(texto: str) -> int:
...
def percentual_acrescimo_por_quantidade(quantidade: int) -> int:
    ...
    if 5001 <= quantidade <= 10000:
        return 35
    raise ValueError("Quantidade fora do intervalo permitido")


def custo_plataforma_centavos(servico: dict, quantidade: int) -> int:
    """Calcula o custo-base em reais usando os valores do catalogo.json como referência.

    Se a quantidade existir no catálogo, usa o valor exato. Se for uma quantidade
    intermediária, calcula proporcionalmente entre os dois pacotes mais próximos.
    """
    itens = []
    for item in servico.get("itens", []):
        try:
            qtd_item = int(item.get("quantidade"))
        except (TypeError, ValueError):
            continue
        valor_item = valor_para_centavos(item.get("valor", "0"))
        if qtd_item > 0 and valor_item > 0:
            itens.append((qtd_item, valor_item, item))

    if not itens:
        raise ValueError("Este serviço não possui valores base configurados no catalogo.json.")

    itens.sort(key=lambda item: item[0])

    for qtd_item, valor_item, _ in itens:
        if quantidade == qtd_item:
            return valor_item

    menor_qtd, menor_valor, _ = itens[0]
    maior_qtd, maior_valor, _ = itens[-1]
    if quantidade < menor_qtd or quantidade > maior_qtd:
        raise ValueError("Quantidade fora da faixa configurada para este serviço.")

    anterior = None
    proximo = None
    for item in itens:
        if item[0] < quantidade:
            anterior = item
        elif item[0] > quantidade:
            proximo = item
            break

    if anterior and proximo:
        qtd_ant, valor_ant, _ = anterior
        qtd_prox, valor_prox, _ = proximo
        proporcao = (quantidade - qtd_ant) / (qtd_prox - qtd_ant)
        return int(round(valor_ant + ((valor_prox - valor_ant) * proporcao)))

    # Segurança extra para catálogos com apenas um item válido.
    return int(round(maior_valor * quantidade / maior_qtd))


def calcular_valor_cliente(servico: dict, quantidade: int) -> dict:
    base_centavos = custo_plataforma_centavos(servico, quantidade)
    acrescimo = percentual_acrescimo_por_quantidade(quantidade)
    valor_cliente_centavos = int(round(base_centavos * (100 + acrescimo) / 100))
    return {
        "valor": centavos_para_moeda(valor_cliente_centavos),
        "valor_plataforma": centavos_para_moeda(base_centavos),
        "acrescimo_percentual": acrescimo,
    }


def mensagem_solicitar_quantidade(servico: dict) -> str:
    return (
        f"{servico.get('mensagem', '').strip()}\n\n"
        "🔢 *Agora envie a quantidade desejada.*\n\n"
        "Digite qualquer quantidade de *100 a 10.000*.\n"
        "Exemplos: `100`, `500`, `1.000`, `2500`.\n\n"
        "O valor será calculado automaticamente em *reais (BRL)* conforme a faixa de quantidade."
    )


def mensagem_quantidade_registrada(catalogo_nome: str, servico: dict, quantidade: int, dados_valor: dict) -> str:
    return (
        "✅ *Quantidade registrada!*\n\n"
        f"🗂️ *Catálogo:* {md(catalogo_nome)}\n"
        f"📌 *Serviço:* {md(servico.get('nome', ''))}\n"
        f"🔢 *Quantidade:* {md(formatar_quantidade(quantidade))}\n"
        f"💰 *Valor:* R$ {md(dados_valor['valor'])}\n\n"
        "Agora envie o *link ou @* do perfil/publicação para continuar o pedido.\n\n"
        "⚠️ Se for perfil, deixe público até a conclusão do pedido."
    )


def botoes_voltar_quantidade(catalogo_key: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([[btn("⬅️ Voltar", f"catalogo:{catalogo_key}")]])
...
def texto_pedido_pendente_admin(pedido: dict) -> str:
    ...
    bloco_calculo = ""
    if pedido.get("valor_plataforma"):
        bloco_calculo = (
            f"💵 *Base plataforma:* R$ {md(pedido.get('valor_plataforma', ''))}\n"
            f"📈 *Acréscimo aplicado:* {md(pedido.get('acrescimo_percentual', ''))}%\n"
        )
    return (
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("servico_tiktok:"):
        ...
        context.user_data["pedido_quantidade"] = {
            "catalogo_key": "tiktok",
            "catalogo_nome": "TikTok",
            "servico_chave": servico_chave,
        }
        await safe_edit_or_reply(update, mensagem_solicitar_quantidade(servico), botoes_voltar_quantidade("tiktok"))
        return
    ...
    if data.startswith("item_tiktok:"):
        ...
        servico = CATALOGO["catalogos"]["tiktok"]["servicos"][servico_chave]
        dados_valor = calcular_valor_cliente(servico, quantidade)
        ...
        context.user_data["pedido"] = preparar_pedido({
            ...
            "valor_plataforma": dados_valor["valor_plataforma"],
            "acrescimo_percentual": dados_valor["acrescimo_percentual"],
            "link": None,
        ...
        await safe_edit_or_reply(
            update,
            mensagem_quantidade_registrada("TikTok", servico, quantidade, dados_valor),
    ...
    if data.startswith("servico:"):
        ...
        context.user_data.pop("pedido", None)
        context.user_data["pedido_quantidade"] = {
            "catalogo_key": "instagram",
            "catalogo_nome": "Instagram",
            "servico_chave": servico_chave,
        }
        await safe_edit_or_reply(update, mensagem_solicitar_quantidade(servico), botoes_voltar_quantidade("instagram"))
        return
    ...
    if data.startswith("item:"):
        ...
        await safe_edit_or_reply(
            update,
            mensagem_quantidade_registrada("Instagram", servico, quantidade, dados_valor),
...
def item_referencia_por_quantidade(servico: dict, quantidade: int) -> dict | None:
    for item in servico.get("itens", []):
        try:
            if int(item.get("quantidade")) == int(quantidade):
                return item
        except (TypeError, ValueError):
            continue
    return None
...
async def processar_quantidade_pedido(update: Update, context: ContextTypes.DEFAULT_TYPE, texto_usuario: str):
    selecao = context.user_data.get("pedido_quantidade") or {}
    catalogo_key = selecao.get("catalogo_key")
    catalogo_nome = selecao.get("catalogo_nome")
    servico_chave = selecao.get("servico_chave")

    try:
        quantidade = normalizar_quantidade_cliente(texto_usuario)
        if quantidade < 100 or quantidade > 10000:
            raise ValueError("Quantidade fora do limite")
    except ValueError:
        await update.message.reply_text(
            "⚠️ Envie uma quantidade válida somente com números, de *100 a 10.000*.\n\n"
            "Exemplo: `500` ou `1.000`.",
            parse_mode=ParseMode.MARKDOWN,
        )
        return

    try:
        servico = CATALOGO["catalogos"][catalogo_key]["servicos"][servico_chave]
        dados_valor = calcular_valor_cliente(servico, quantidade)
    except Exception as exc:
        await update.message.reply_text(
            "⚠️ Não consegui calcular o valor dessa quantidade.\n\n"
            f"*Motivo:* {md(exc)}\n\n"
            "Tente outra quantidade ou volte ao catálogo.",
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=botoes_voltar_quantidade(catalogo_key or "instagram"),
        )
        return

    item_ref = item_referencia_por_quantidade(servico, quantidade) or {}
    context.user_data.pop("pedido_quantidade", None)
    context.user_data["pedido"] = preparar_pedido({
        ...
        "quantidade": formatar_quantidade(quantidade),
        "quantidade_api": quantidade,
        "api_service_id": item_ref.get("api_service_id") or servico.get("api_service_id"),
        "valor": dados_valor["valor"],
        "valor_plataforma": dados_valor["valor_plataforma"],
        "acrescimo_percentual": dados_valor["acrescimo_percentual"],
        "link": None,
    ...
    await enviar_texto_sequencial(
        update,
        context,
        mensagem_quantidade_registrada(catalogo_nome, servico, quantidade, dados_valor),
        InlineKeyboardMarkup([[btn("⬅️ Voltar", f"servico_{catalogo_key}:{servico_chave}" if catalogo_key == "tiktok" else f"servico:{servico_chave}")]]),
    )
...
async def receber_texto(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if context.user_data.get("pedido_quantidade"):
        await processar_quantidade_pedido(update, context, texto_usuario)
        return
