...
def quebrar_texto(draw, texto: str, fonte, largura_max: int) -> list[str]:
...
def desenhar_texto_centralizado(draw, y: int, linhas, fonte, cor, largura: int, espacamento: int = 8) -> int:
    ...
    for linha in linhas:
        ...
        w = bbox[2] - bbox[0]
        h = bbox[3] - bbox[1]
        draw.text(((largura - w) / 2, y), linha, font=fonte, fill=cor)
        y += h + espacamento
    return y
...
def gerar_imagem_pagamento_instagram(pedido: dict) -> BytesIO | None:
    ...
    def fonte_ajustada(texto: str, caixa, tamanho_max: int, tamanho_min: int = 18, negrito: bool = True):
        x1, y1, x2, y2 = caixa
        texto = str(texto or "").strip()
        largura_max = max(10, x2 - x1 - int(20 * sx))
        altura_max = max(10, y2 - y1 - int(10 * sy))

        for tamanho in range(tamanho_max, tamanho_min - 1, -2):
            fonte = fonte_pagamento(int(tamanho * min(sx, sy)), negrito)
            bbox = draw.textbbox((0, 0), texto, font=fonte)
            if (bbox[2] - bbox[0]) <= largura_max and (bbox[3] - bbox[1]) <= altura_max:
                return fonte, texto

        fonte = fonte_pagamento(int(tamanho_min * min(sx, sy)), negrito)
        texto_final = texto
        while texto_final:
            teste = texto_final + "…"
    ...
    def escrever_caixa(texto: str, caixa_base, tamanho_max: int, cor=(245, 245, 245), negrito: bool = True):
        caixa = escala_caixa(caixa_base)
        x1, y1, x2, y2 = caixa
        fonte, texto_final = fonte_ajustada(texto, caixa, tamanho_max, negrito=negrito)
        bbox = draw.textbbox((0, 0), texto_final, font=fonte)
        tw = bbox[2] - bbox[0]
        th = bbox[3] - bbox[1]
        tx = x1 + ((x2 - x1) - tw) / 2
        ty = y1 + ((y2 - y1) - th) / 2 - bbox[1]
        draw.text((tx, ty), texto_final, font=fonte, fill=cor)
    ...
    link = str(pedido.get("link", "")).strip()

    # Mantém o layout exatamente como imagem base e apenas escreve nos campos tracejados.
    # Caixas medidas no layout original 1024x1536 enviado pelo cliente.
    escrever_caixa(f"R$ {valor}", (306, 612, 542, 698), 38, cor=(255, 255, 255), negrito=True)
    escrever_caixa(catalogo, (333, 883, 625, 956), 31, cor=(255, 255, 255), negrito=True)
    escrever_caixa(servico, (335, 978, 626, 1048), 30, cor=(255, 255, 255), negrito=True)
    escrever_caixa(quantidade, (337, 1071, 618, 1144), 32, cor=(255, 255, 255), negrito=True)
    escrever_caixa(link, (366, 1163, 626, 1238), 25, cor=(255, 255, 255), negrito=True)

    # Se a chave Pix for alterada no .env, substitui apenas o texto da chave, mantendo o ícone e a borda do layout.
    if PIX_CHAVE and PIX_CHAVE != "ttwovendas@gmail.com":
        apagar_area((201, 476, 640, 535), margem=2)
        escrever_caixa(PIX_CHAVE, (201, 466, 640, 545), 34, cor=(255, 255, 255), negrito=True)
