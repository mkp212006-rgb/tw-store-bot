...
def menu_principal() -> InlineKeyboardMarkup:
    keyboard = [
        [btn("🛍️ Comprar serviço", "menu:catalogo")],
        [btn("🔎 Consultar pedido", "pedido:consultar")],
        [btn("💬 Suporte / Atendimento", "extra:atendimento")],
        [btn("❓ Como funciona", "extra:como_fazer_pedido")],
    ]
    return InlineKeyboardMarkup(keyboard)
...
def menu_catalogos() -> InlineKeyboardMarkup:
    keyboard = [
        ...
        [btn("⬅️ Voltar ao início", "voltar:inicio")],
    ]
    return InlineKeyboardMarkup(keyboard)
...
def obter_servico_social(catalogo_chave: str, servico_chave: str) -> dict:
    return CATALOGO["catalogos"][catalogo_chave]["servicos"][servico_chave]


def get_item_social(catalogo_chave: str, servico_chave: str, quantidade: int) -> dict:
    servico = obter_servico_social(catalogo_chave, servico_chave)
    for item in servico["itens"]:
        if int(item["quantidade"]) == int(quantidade):
            return item
    raise KeyError("Item não encontrado")
...
def nome_catalogo_social(catalogo_chave: str) -> str:
    return "TikTok" if catalogo_chave == "tiktok" else "Instagram"


def callback_servico_social(catalogo_chave: str, servico_chave: str) -> str:
    prefixo = "servico_tiktok" if catalogo_chave == "tiktok" else "servico"
    return f"{prefixo}:{servico_chave}"


def callback_voltar_catalogo_social(catalogo_chave: str) -> str:
    return f"catalogo:{catalogo_chave}"


def parse_quantidade_digitada(texto: str) -> int | None:
    texto = str(texto or "").strip().lower()
    normalizado = re.sub(r"[\s.,]", "", texto)

    if re.fullmatch(r"\d+", normalizado):
        return int(normalizado)

    if re.fullmatch(r"\d+k", normalizado):
        return int(normalizado[:-1]) * 1000

    return None


def texto_escolher_quantidade(catalogo_chave: str, servico_chave: str) -> str:
    servico = obter_servico_social(catalogo_chave, servico_chave)
    linhas = [
        servico.get("mensagem", "").strip(),
        "",
        "🔢 *Escolha a quantidade*",
        "",
        "Digite exatamente uma das opções disponíveis abaixo:",
        "",
    ]

    for item in servico["itens"]:
        linhas.append(f'• {md(item["quantidade_texto"])} — {md(money(item["valor"]))}')

    linhas.extend([
        "",
        "Exemplo: `100`, `500` ou `1000`.",
    ])
    return "\n".join(linha for linha in linhas if linha is not None)


def mensagem_quantidade_invalida(catalogo_chave: str, servico_chave: str) -> str:
    servico = obter_servico_social(catalogo_chave, servico_chave)
    opcoes = ", ".join(str(item["quantidade_texto"]) for item in servico["itens"])
    return (
        "❌ *Quantidade indisponível.*\n\n"
        "Para continuar, envie uma das quantidades cadastradas para este serviço:\n"
        f"`{md(opcoes)}`\n\n"
        "Assim o pedido fica certinho com o valor correto do catálogo."
    )


def validar_dado_pedido(pedido: dict, texto: str) -> tuple[bool, str]:
    texto = str(texto or "").strip()
    catalogo = pedido.get("catalogo", "")

    if not texto or len(texto) < 3:
        return False, "Envie uma informação válida para continuar."

    if catalogo in ("IPTV Livestream 4K", "Internet Ilimitada"):
        if re.fullmatch(r"[^@\s]+@[^@\s]+\.[^@\s]+", texto):
            return True, ""
        return False, "Envie um e-mail válido para ativação do serviço. Exemplo: nome@email.com"

    if catalogo in ("Instagram", "TikTok"):
        if " " in texto:
            return False, "Envie apenas o link ou @, sem frases junto."
        if texto.startswith("@") or texto.startswith("http://") or texto.startswith("https://") or ".com" in texto:
            return True, ""
        # Também permite usuário simples sem @, mantendo compatibilidade com pedidos antigos.
        if re.fullmatch(r"[A-Za-z0-9._-]{3,80}", texto):
            return True, ""
        return False, "Envie um @ ou link válido para esse pedido."

    return True, ""


def mensagem_dado_invalido(motivo: str) -> str:
    return (
        "❌ *Não consegui validar a informação enviada.*\n\n"
        f"{md(motivo)}\n\n"
        "Revise e envie novamente para continuar."
    )


def montar_pedido_social(update: Update, catalogo_chave: str, servico_chave: str, item: dict) -> dict:
    servico = obter_servico_social(catalogo_chave, servico_chave)
    return preparar_pedido({
        "catalogo": nome_catalogo_social(catalogo_chave),
...
async def pedir_quantidade_social(update: Update, context: ContextTypes.DEFAULT_TYPE, catalogo_chave: str, servico_chave: str):
    # Novo pedido social: escolhe o serviço no botão e digita a quantidade.
    # A validação usa somente os pacotes existentes no catalogo.json.
    context.user_data.pop("pedido", None)
    context.user_data["aguardando_quantidade_social"] = {
        "catalogo_chave": catalogo_chave,
        "servico_chave": servico_chave,
    }
    await safe_edit_or_reply(
        update,
        texto_escolher_quantidade(catalogo_chave, servico_chave),
        InlineKeyboardMarkup([[btn("⬅️ Voltar", callback_voltar_catalogo_social(catalogo_chave))]]),
    )


async def processar_quantidade_social_digitada(update: Update, context: ContextTypes.DEFAULT_TYPE, texto_usuario: str) -> bool:
    estado = context.user_data.get("aguardando_quantidade_social")
    if not estado:
        return False

    catalogo_chave = estado.get("catalogo_chave")
    servico_chave = estado.get("servico_chave")
    quantidade = parse_quantidade_digitada(texto_usuario)

    try:
        if quantidade is None:
            raise KeyError("Quantidade inválida")
        item = get_item_social(catalogo_chave, servico_chave, quantidade)
    except Exception:
        await update.message.reply_text(
            mensagem_quantidade_invalida(catalogo_chave, servico_chave),
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=InlineKeyboardMarkup([[btn("⬅️ Voltar", callback_voltar_catalogo_social(catalogo_chave))]]),
        )
        return True

    context.user_data.pop("aguardando_quantidade_social", None)
    context.user_data["pedido"] = montar_pedido_social(update, catalogo_chave, servico_chave, item)

    await update.message.reply_text(
        item["mensagem"],
        parse_mode=ParseMode.MARKDOWN,
        reply_markup=InlineKeyboardMarkup([[btn("⬅️ Voltar", callback_servico_social(catalogo_chave, servico_chave))]]),
        disable_web_page_preview=True,
    )
    return True
...
def texto_pagamento(pedido: dict) -> str:
    ...
    if pedido.get("mp_qr_code"):
        ...
        return (
            ...
            "Confira o resumo do pedido e pague pelo Pix copia e cola.\n"
            "Após o pagamento, a confirmação é feita automaticamente.\n"
            ...
            "Toque no botão abaixo para copiar o Pix. Depois do pagamento, toque em *Já paguei / verificar* ou aguarde a confirmação automática."
        )
    ...
    if mercado_pago_configurado():
        ...
        if not ok:
            await enviar_texto_sequencial(
                ...
                context,
                (
                    "⚠️ Não consegui gerar o Pix automático agora.\n\n"
                    "Tente novamente em instantes ou fale com o atendimento para finalizar seu pedido."
                ),
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("servico_tiktok:"):
        ...
        await pedir_quantidade_social(update, context, "tiktok", servico_chave)
        return
    ...
    if data.startswith("servico:"):
        servico_chave = data.split(":", 1)[1]
        await pedir_quantidade_social(update, context, "instagram", servico_chave)
        return
...
async def receber_texto(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if await processar_quantidade_social_digitada(update, context, texto_usuario):
        return
    ...
    if not pedido.get("link"):
        valido, motivo = validar_dado_pedido(pedido, texto_usuario)
        if not valido:
            await update.message.reply_text(
                mensagem_dado_invalido(motivo),
                parse_mode=ParseMode.MARKDOWN,
                reply_markup=InlineKeyboardMarkup([[btn("⬅️ Voltar ao início", "voltar:inicio")]]),
                disable_web_page_preview=True,
            )
            return
...
async def admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not eh_admin(update):
        await update.effective_message.reply_text("Comando disponível apenas para o administrador.")
        return

    pendentes = carregar_pedidos_pendentes()
    historico = carregar_pedidos_historico()
    semana = carregar_totais_semanais()
    total_semana = sum(int(c.get("total_centavos", 0)) for c in semana.get("clientes", {}).values())

    texto = (
        "🛠️ *Painel administrativo — TW STORE*\n\n"
        f"⏳ *Pedidos pendentes:* {len(pendentes)}\n"
        f"✅ *Pedidos no histórico:* {len(historico)}\n"
        f"💰 *Total da semana:* R$ {md(centavos_para_moeda(total_semana))}\n"
        f"🗓️ *Semana:* {md(semana.get('inicio', ''))} até {md(semana.get('fim', ''))}\n\n"
        "Use este painel para acompanhar o movimento geral da loja. "
        "Os relatórios completos continuam sendo enviados automaticamente quando um pedido é aprovado."
    )
