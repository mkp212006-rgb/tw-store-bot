...
USUARIOS_REGISTRADOS_PATH = BASE_DIR / "usuarios_registrados.json"
...
async def aprovar_registro_admin(update: Update, context: ContextTypes.DEFAULT_TYPE, telegram_id: str):
    ...
    registro["aprovado_por"] = update.effective_user.full_name if update.effective_user else "Administrador"
    registro.pop("tentar_novamente_em", None)
    usuarios[str(telegram_id)] = registro
    salvar_usuarios_registrados(usuarios)
    await query.answer("Cadastro aprovado.")
    await query.message.reply_text(f"✅ Cadastro `{md(telegram_id)}` aprovado.", parse_mode=ParseMode.MARKDOWN)
    try:
        await context.bot.send_message(
            chat_id=telegram_id,
...
async def negar_registro_admin(update: Update, context: ContextTypes.DEFAULT_TYPE, telegram_id: str):
    ...
    registro["negado_por"] = update.effective_user.full_name if update.effective_user else "Administrador"
    registro["tentar_novamente_em"] = tentar_em.isoformat()
    usuarios[str(telegram_id)] = registro
    salvar_usuarios_registrados(usuarios)
    await query.answer("Cadastro negado.")
    await query.message.reply_text(
        f"❌ Cadastro `{md(telegram_id)}` negado. O cliente poderá tentar novamente após 30 minutos.",
        parse_mode=ParseMode.MARKDOWN,
    )
    try:
        await context.bot.send_message(
