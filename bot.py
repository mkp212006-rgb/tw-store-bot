...
def menu_items(servico_chave: str) -> InlineKeyboardMarkup:
    servico = CATALOGO["catalogos"]["instagram"]["servicos"][servico_chave]
    keyboard = []
    for item in servico["items"]:
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("extra:"):
        ...
        if extra == "atendimento":
            keyboard.insert(0, [btn("💬 Pagamento", "extra:pagamento")])
        await safe_edit_or_reply(update, texto, InlineKeyboardMarkup(keyboard))
        return

    if data == "extra:pagamento":
        await safe_edit_or_reply(
            update,
            CATALOGO["mensagens"].get("pagamento", "Entre em contato com o atendimento para realizar o pagamento."),
            InlineKeyboardMarkup([[btn("⬅️ Voltar", "extra:atendimento")]]),
        )
        return
    ...
    if data == "extra_confirmado":
        await safe_edit_or_reply(
            update,
            CATALOGO["mensagens"].get("pedido_confirmado", "✅ Pedido confirmado!"),
    ...
    if data.startswith("servico:"):
        ...
        servico = CATALOGO["catalogos"]["instagram"]["servicos"][servico_chave]
        await safe_edit_or_reply(update, servico["mensagem"], menu_items(servico_chave))
        return
    ...
    if data == "confirmar_pedido":
        ...
        if not pedido:
            await safe_edit_or_reply(
                update,
                "Não encontrei um pedido em andamento. Toque em /start para começar novamente.",
                menu_principal(),
            )
            return
        ...
        if not pedido.get("link"):
            await safe_edit_or_reply(
                update,
                "Envie primeiro o link do perfil para continuar.",
        ...
        await safe_edit_or_reply(
            update,
            CATALOGO["mensagens"].get("pedido_confirmado", "✅ Pedido confirmado! Em breve nossa equipe dará continuidade."),
            InlineKeyboardMarkup([[btn("🏠 Menu inicial", "voltar:inicio")]]),
        )
        context.user_data.clear()
        return
...
async def receber_texto(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not pedido.get("link"):
        ...
        resumo_cliente = (
            "✅ *Link recebido!*\n\n"
            "📋 *Resumo do pedido:*\n"
            f"• Catálogo: {pedido.get('catalogo', 'Não informado')}\n"
            f"• Serviço: {pedido.get('servico', 'Não informado')}\n"
            f"• Quantidade: {pedido.get('quantidade', 'Não informada')}\n"
            f"• Valor: R$ {pedido.get('valor', '0,00')}\n"
            f"• Link enviado: {pedido.get('link')}\n\n"
            "Agora toque em ✅ *Confirmar pedido* para ir para a etapa de pagamento."
        )
    ...
    await update.message.reply_text(
        "Você já enviou o link. Agora toque em ✅ Confirmar pedido para finalizar.",
        reply_markup=InlineKeyboardMarkup([
            [btn("✅ Confirmar pedido", "confirmar_pedido")]
        ]),
    )
...
async def enviar_relatorio_admin(update: Update, context: ContextTypes.DEFAULT_TYPE, pedido: dict):
    ...
    relatorio = (
        ...
        f"💰 *Valor:* R$ {pedido.get('valor')}\n"
        f"🔗 *Link:* {pedido.get('link')}\n"
        f"👤 *Cliente:* {pedido.get('usuario')}\n"
        f"💬 *Telegram:* {username}\n"
