...
def menu_painel_admin() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [btn("📊 Resumo", "admin_painel:resumo"), btn("🧾 Últimos pedidos", "admin_painel:ultimos")],
...
def texto_painel_admin() -> str:
    return (
        ...
        "Gerencie cadastros, pedidos, pagamentos pendentes, usuários e bloqueios."
    )
...
def formatar_usuario_admin(telegram_id: str, registro: dict) -> str:
    ...
    return (
        f"• `{md(telegram_id)}` — *{md(status)}*\n"
        f"  {md(nome)} | {md(username)}\n"
        f"  ID interno: `{md(usuario_login)}` | Criado: {md(criado)}"
    )
...
def texto_ultimos_pedidos_admin() -> str:
    ...
    itens = sorted(historico.items(), key=chave, reverse=True)[:15]
    linhas = ["🧾 *Últimos pedidos finalizados*\n"]
    for pedido_id, pedido in itens:
        linhas.append(
            f"• `{md(pedido_id)}` — R$ {md(pedido.get('valor', ''))}\n"
            f"  {md(pedido.get('catalogo', ''))} | {md(pedido.get('servico', ''))}\n"
            f"  Cliente: {md(pedido.get('usuario', ''))} | Status: {md(traduzir_status_local(pedido.get('status')))}\n"
            f"  Plataforma: `{md(pedido.get('plataforma_order_id', 'Não informado'))}`"
        )
    return "\n".join(linhas)
...
async def mostrar_resumo_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not eh_admin(update):
        await update.callback_query.answer("Apenas administradores podem ver o resumo.", show_alert=True)
        return
...
async def mostrar_cadastros_pendentes_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not eh_admin(update):
        ...
        return
    await safe_edit_or_reply(update, texto_cadastros_pendentes_admin(), menu_voltar_painel_admin())
...
async def processar_texto_admin_painel(update: Update, context: ContextTypes.DEFAULT_TYPE, texto_usuario: str) -> bool:
    ...
    if acao == "buscar_usuario":
        ...
        await update.message.reply_text(
            resposta,
            parse_mode=ParseMode.MARKDOWN,
            reply_markup=menu_voltar_painel_admin(),
            disable_web_page_preview=True,
        )
        return True
