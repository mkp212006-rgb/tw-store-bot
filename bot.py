...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "confirmar_email_iptv":
        pedido = context.user_data.get("pedido")
        if not pedido or pedido.get("catalogo") != "IPTV Livestream 4K" or not pedido.get("link"):
    ...
    if data == "alterar_link":
        ...
        if not pedido:
            ...
            return
        pedido["link"] = None
        if pedido.get("catalogo") == "IPTV Livestream 4K":
            pedido["status"] = "aguardando_email_iptv"
            await safe_edit_or_reply(update, "✏️ Envie novamente o e-mail correto para continuar.")
        else:
...
async def receber_texto(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not pedido.get("link"):
        ...
        if pedido.get("catalogo") == "IPTV Livestream 4K" and pedido.get("status") == "aguardando_email_iptv":
            await update.message.reply_text(
                (
