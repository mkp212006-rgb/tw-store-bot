...
from validators import validar_destino_pedido, validar_link_social_antes_pagamento
...
def envio_plataforma_bloqueado_para_auto(pedido: dict) -> bool:
    ...
    return status_api in {"processando", "revisao_manual", "erro", "ignorado_manual", "resolvido_manual"}
...
async def verificar_link_social_antes_pagamento(update: Update, context: ContextTypes.DEFAULT_TYPE, pedido: dict) -> bool:
    """Confere se link/perfil está público antes de liberar a aba de pagamento."""
    if not pedido:
        return False

    if pedido.get("link_validado_antes_pagamento"):
        return True

    try:
        ok, detalhe = await asyncio.to_thread(validar_link_social_antes_pagamento, pedido)
    except Exception as exc:
        ok = False
        detalhe = f"⚠️ Erro inesperado ao verificar o link: {limpar_erro_api(exc)}"

    if ok:
        pedido["link_validado_antes_pagamento"] = True
        pedido["ultima_verificacao_link"] = detalhe
        return True

    pedido["status"] = "aguardando_link"
    pedido["motivo_bloqueio_link"] = detalhe
    pedido["link_validado_antes_pagamento"] = False
    pedido["link"] = None
    pedido.pop("mp_payment_id", None)
    pedido.pop("mp_qr_code", None)
    pedido.pop("mp_ticket_url", None)
    pedido.pop("pagamento_criado_em", None)
    pedido.pop("pagamento_expira_em", None)
    ...
    await enviar_texto_sequencial(
        ...
        InlineKeyboardMarkup([
            [btn("✏️ Enviar outro link/@", "alterar_link")],
...
async def enviar_pagamento_cliente(update: Update, context: ContextTypes.DEFAULT_TYPE, pedido: dict):
    ...
    if not await verificar_link_social_antes_pagamento(update, context, pedido):
        return
