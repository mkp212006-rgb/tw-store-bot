...
def cargo_usuario_id(telegram_id, registro: dict | None = None) -> str:
    """Resolve o cargo efetivo, incluindo os dois administradores de bootstrap."""
    telegram_id = str(telegram_id or "").strip()
    if telegram_id and telegram_id == str(ADMIN_CHAT_ID or "").strip():
        return CARGO_DONO
    if telegram_id and telegram_id == str(ADMIN2_CHAT_ID or "").strip():
...
def id_administrador_sistema(telegram_id) -> bool:
    telegram_id = str(telegram_id or "").strip()
    return telegram_id in ids_unicos(ADMIN_CHAT_ID, ADMIN2_CHAT_ID)
...
def ids_com_permissao(permissao: str) -> list[str]:
    ids = ids_unicos(ADMIN_CHAT_ID, ADMIN2_CHAT_ID)
...
async def aprovar_registro_admin(update: Update, context: ContextTypes.DEFAULT_TYPE, telegram_id: str):
    ...
    registro["cargo"] = normalizar_cargo(registro.get("cargo"), CARGO_VENDEDOR)
...
def menu_painel_admin() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [btn("🪪 Aplicar Cargos", "admin_painel:cargos")],
    ])
...
def texto_gerenciar_cargos_admin() -> str:
    return (
        "🪪 *Aplicar cargos*\n\n"
        "Envie o *Telegram ID* numérico de um usuário registrado e aprovado.\n\n"
...
def menu_escolher_cargo_admin(telegram_id: str, cargo_autor: str) -> InlineKeyboardMarkup:
    keyboard = []
    for cargo_chave, cargo_info in CARGOS.items():
        if pode_atribuir_cargo(cargo_autor, cargo_chave):
            keyboard.append([
                btn(
                    f"🪪 {cargo_info['nome']}",
                    f"admin_cargo:{cargo_chave}:{telegram_id}",
                )
            ])
    keyboard.append([btn("⬅️ Voltar ao painel", "admin_painel:inicio")])
    return InlineKeyboardMarkup(keyboard)
...
async def mostrar_gerenciar_cargos_admin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not usuario_tem_permissao_update(update, PERMISSAO_GERENCIAR_CARGOS):
        await update.callback_query.answer("Seu cargo não pode aplicar cargos.", show_alert=True)
        return
    context.user_data.clear()
    context.user_data["admin_acao_usuario"] = "aplicar_cargo"
    await safe_edit_or_reply(
        update,
        texto_gerenciar_cargos_admin(),
        menu_voltar_cargos_admin(),
    )
...
async def aplicar_cargo_admin(
    ...
    if id_administrador_sistema(telegram_id):
        await query.answer("Os administradores configurados no servidor têm cargo protegido.", show_alert=True)
        return
    ...
    registro["cargo"] = cargo_novo
    registro["cargo_aplicado_em"] = agora_br().strftime("%d/%m/%Y %H:%M:%S")
    registro["cargo_aplicado_por"] = (
    ...
    if cargo_novo == CARGO_TESTER:
        ...
        registro["meta_tester_semana_inicio"] = semana_info()["id"]
    else:
...
async def desbanir_telegram_id_pelo_painel(update: Update, context: ContextTypes.DEFAULT_TYPE, telegram_id: str) -> str:
    ...
    if not registro:
        return "⚠️ Não encontrei esse Telegram ID no cadastro do bot."
    if id_administrador_sistema(telegram_id):
        return "⚠️ Os administradores configurados no servidor têm cargo protegido."
    if not pode_gerenciar_cargo(cargo_usuario_update(update), cargo_usuario_id(telegram_id, registro)):
...
async def processar_texto_admin_painel(update: Update, context: ContextTypes.DEFAULT_TYPE, texto_usuario: str) -> bool:
    ...
    if acao == "aplicar_cargo":
        ...
        if id_administrador_sistema(telegram_id):
            await update.message.reply_text(
                "⚠️ Esse ID pertence a um administrador configurado no servidor e tem cargo protegido.",
                reply_markup=menu_voltar_cargos_admin(),
            )
            return True
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("admin_cargo:"):
        _, cargo_novo, telegram_id = data.split(":", 2)
        await aplicar_cargo_admin(update, context, cargo_novo, telegram_id)
        return
