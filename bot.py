...
def menu_painel_admin() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        ...
        [btn("📒 Consultar Cadrastos", "admin_painel:consultar_cadastros")],
...
def envio_plataforma_estava_processando(pedido: dict) -> bool:
    ...
    if not pedido:
        return False
    status_api = str(pedido.get("plataforma_api_status") or "").strip().lower()
    return status_api == "processando" and not pedido_ja_enviado_para_plataforma(pedido)


def marcar_envio_plataforma_para_revisao_manual(pedido: dict, origem: str = "restart"):
    ...
    pedido["plataforma_api_erro"] = (
        ...
        "enquanto este pedido estava sendo enviado para a plataforma. "
        "Confira no painel da plataforma se o pedido já foi criado antes de reenviar manualmente."
    )
...
def corrigir_pedidos_com_envio_interrompido():
    ...
    for pedido_id, pedido in list(carregar_pedidos_pendentes().items()):
        if pedido.get("catalogo") not in CATALOGOS_COM_ENVIO_API:
            continue
        if not envio_plataforma_estava_processando(pedido):
            continue
...
def enviar_relatorio_admin_documento_sync(pedido: dict, total_semanal_cliente: str, titulo: str = "NOVO PEDIDO PAGO — TW STORE") -> bool:
    ...
    for admin_chat_id in destinatarios:
        enviado = enviar_documento_telegram_sync(admin_chat_id, imagem, caption_relatorio_admin(pedido, titulo)) or enviado
    return enviado
...
def processar_pagamento_aprovado_sync(pedido: dict, pagamento: dict, origem: str = "webhook") -> bool:
    ...
    try:
        ...
        if pedido.get("catalogo") in CATALOGOS_COM_ENVIO_API:
            ...
            elif status_api_antes == "processando":
                marcar_envio_plataforma_para_revisao_manual(pedido, origem=f"{origem}_restart_guard")
                salvar_pedido_pendente(pedido)
            else:
        ...
        if destinatarios_relatorio_pedido:
            ...
            if not enviado_documento:
                ...
                for admin_chat_id in destinatarios_relatorio_pedido:
                    ...
                    enviar_telegram_sync(admin_chat_id, relatorio_parte)
...
def menu_instagram() -> InlineKeyboardMarkup:
    keyboard = [
        [btn("🌐 Estrangeiros", "catalogo_instagram:estrangeiros")],
        [btn("🇧🇷 Brasileiros", "catalogo_instagram:brasileiros")],
        [btn("⬅️ Voltar", "catalogo:redes_sociais")],
    ]
    return InlineKeyboardMarkup(keyboard)
...
def menu_tiktok() -> InlineKeyboardMarkup:
    keyboard = [
        [btn("🌐 Estrangeiros", "catalogo_tiktok:estrangeiros")],
        [btn("⬅️ Voltar", "catalogo:redes_sociais")],
    ]
    return InlineKeyboardMarkup(keyboard)
...
async def callbacks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "catalogo_instagram:brasileiros":
        await safe_edit_or_reply(
            update,
            "🇧🇷 *Instagram Brasileiros*\n\nEssa categoria ainda não possui serviços cadastrados.",
            InlineKeyboardMarkup([[btn("⬅️ Voltar", "catalogo:instagram")]]),
        )
        return
